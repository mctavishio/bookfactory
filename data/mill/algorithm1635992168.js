...
const algorithmtimestamp = "1635992168";
...
seed.dimensions.forEach( dimension => {
	...
	let matrix = [...Array(seed.npages).keys()].reduce( (pagematrix, pagej) => {
		...
		let colors = tools.reifyWeightedArray([ [tools.pigments.black,8], [tools.pigments.white,12], [tools.pigments.blue,0], [tools.pigments.yellow,0], [tools.pigments.red,2]]);
		...
		let layers = layersmill.reduce( (layermatrix, layerj) => {
				...
				let dash = mill.map(n=>tools.randominteger(0.1*min,0.8*min)).sort( (a,b) => b-a );
				let space = mill.map(n=>tools.randominteger(0.1*min,0.8*min)).sort( (a,b) => b-a );
	...
	matrix.forEach( (page,p) => {
		let colors = tools.reifyWeightedArray([ [tools.pigments.black,2],  [tools.pigments.white,4], [tools.pigments.blue,0], [tools.pigments.yellow,1], [tools.pigments.red,0]]);
		// layer 0 ::: background canvas
		let color1 = colors[tools.randominteger(0,colors.length)];
		let notcolor1 = colors.filter(color => color!==color1);
		let color2 = notcolor1[tools.randominteger(0,notcolor1.length)];
		...
		// doc.rect(0, 0, width, height).fillOpacity(1.0).fillColor(color1).fill();
		// doc.rect(0, 0, width, height).strokeOpacity(1.0).strokeColor(color2).dash(dash, {space:space}).lineWidth(lineWidth).stroke();
		// doc.rect(0, 0, width, height).strokeOpacity(1.0).strokeColor(color1).dash(space, {space:dash}).lineWidth(lineWidth*0.4).stroke();
		...
		// let x = [-0.05*width,0,0.05*width][tools.randominteger(0,3)];
		// let y = [-0.05*height,0,0.05*height][tools.randominteger(0,3)];
		// doc.rect(x, y, width, height).strokeOpacity(1.0).fillOpacity(0).strokeColor(color2).dash(space*0.8, {space:dash*0.8}).lineWidth(lineWidth*0.8).stroke();
		// doc.rect(x, y, width, height).strokeOpacity(1.0).fillOpacity(0).strokeColor(color1).dash(dash*0.8, {space:space*0.8}).lineWidth(lineWidth*0.5).stroke();
